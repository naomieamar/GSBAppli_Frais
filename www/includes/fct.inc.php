...
function estEntierPositif($valeur)
{
    ...
    //preg_match verifie que la valeur contient bien des chiffres entre 0 et 9
}
...
function estDateValide($date)
{
    ...
    // explode c'est retirer : de date j'enleve tout les /
    $dateOK = true;
    if (count($tabDate) != 3) {
        $dateOK = false;
        //verifie qu'il y a bien 3 colones
    } else {
        if (!estTableauEntiers($tabDate)) {
            $dateOK = false;
            // verifie que les nombres sont entre 0 et 9
        } else {
            if (!checkdate($tabDate[1], $tabDate[0], $tabDate[2])) {
                $dateOK = false;
                // checkdate verifie que la date est bien gregorienne 
            }
        }
    }
    return $dateOK;
